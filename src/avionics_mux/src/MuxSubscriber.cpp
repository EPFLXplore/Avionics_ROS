...
MuxSubscriber::MuxSubscriber(CANBus* bus, rclcpp::Node* parent) : bus(bus), parent(parent) {
    ...
    // this->mass_config_req_sub = parent->create_subscription<custom_msg::msg::<MassArray>
    //     (get_prefix() + get_param<std::string>("MASS_REQ_ARRAY"), 10, std::bind(&MuxSubscriber::massDrillCalibOffsetCallback, this, _1));
